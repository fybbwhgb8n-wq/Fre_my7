...
class GradualWarmupScheduler(_LRScheduler):
    ...
    The optimizer has a base learning rate (base lr).
    When multiplier > 1, warmup will gradually increase lr from base_lr to multiplier*base_lr
    within total_epoch, then start normal scheduler.
    When multiplier == 1.0, warmup will gradually increase lr from 0 to base_lr
    within total_epoch, then start normal scheduler.
    ...
    def get_lr(self):
        if self.last_epoch > self.total_epoch:
            if self.after_scheduler and (not self.finished):
                    ...
                    self.finished = True
                # Important: need to return new base-lr directly
